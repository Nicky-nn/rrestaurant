 ...
 * @param places
 */
export const invRound = (value: number, places = 7) => {
...
export const articuloInventarioToOperacion = (
  ...
  if (articulo.inventario.length === 0) {
    console.warn(
      "Operación cancelada: El artículo no existe o requiere inventario[0]",
    );
    return null;
  }
  const {
  ...
  if (!articuloPrecio) {
    console.warn(
      "Error: No se ha encontrado equivalencia de precio para la unidad solicitada",
    );
    return null;
  }
  ...
  let alm = null;
  if (codigoAlmacen) {
    alm = articulo.inventario[0].detalle.find(
      (i) => i.almacen.codigoAlmacen === codigoAlmacen,
    );
    if (!alm) {
      console.warn(
        `Error: Almacén ${codigoAlmacen} no encontrado en el inventario del artículo`,
      );
      return null;
    }
  }
  ...
  const almacen: AlmacenInventarioProps | null = alm
    ? {
        ...alm.almacen, // Simplificación si la interfaz AlmacenInventarioProps coincide con alm.almacen
  ...
  return {
    ...
    totalComprometido: invRound(
      articulo.inventario[0].totalComprometido / cantidadBase,
    ),
    totalSolicitado: invRound(
      articulo.inventario[0].totalSolicitado / cantidadBase,
    ),
    totalDisponible: invRound(
      articulo.inventario[0].totalDisponible / cantidadBase,
    ),
    stock: lote ? lote.stock : invRound(alm?.stock || 0),
    comprometido: lote ? lote.comprometido : invRound(alm?.comprometido || 0),
    solicitado: lote ? lote.solicitado : invRound(alm?.solicitado || 0),
    disponible: lote ? lote.disponible : invRound(alm?.disponible || 0),
  };
};
