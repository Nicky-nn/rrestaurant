...
interface RouteGuardProps {
    children: ReactNode
    /**
     * Si es true, mostrará logs en la consola para depuración.
     * @default false
     */
    isDebug?: boolean
}
...
/**
 * Componente que protege TODAS las rutas automáticamente
 * Verifica permisos según la ruta actual y los menús configurados
 */
export const RouteGuard: FC<RouteGuardProps> = ({
    children,
    isDebug = false, // Parámetro añadido con valor por defecto false
}) => {
    ...
    useEffect(() => {
        ...
        // Normalizar path para comparación (siempre con / al inicio)
        ...
        const findPermissionForRoute = (items: any[], parentName?: string): boolean => {
            for (const item of items) {
                // Si el item tiene hijos, buscar recursivamente
                if (item.children && item.children.length > 0) {
                    for (const child of item.children) {
                        ...
                        if (childPath === currentPath) {
                            ...
                            if (isDebug) {
                                console.log(
                                    `Ruta encontrada: ${currentPath} -> Permiso: ${requiredPermission}`,
                                )
                            }
                            return true
                        }
                ...
                // Si el item no tiene hijos y coincide con la ruta
                else if (item.path) {
        ...
        // Si la ruta NO está en los menús configurados, DENEGAR acceso por seguridad
        if (!routeFound || !requiredPermission) {
            if (isDebug) {
                console.warn(
                    `Ruta ${location.pathname} no encontrada en menús. Denegando acceso.`,
                )
            }
            setHasAccess(false)
            return
    ...
    }, [location.pathname, permisos, loading, navigations, isDebug])

    if (isDebug) {
        console.log('Ruta actual:', location.pathname)
        console.log('Estado de acceso:', hasAccess)
    ...
    if (!hasAccess) {
        return (
            ...
            >
                <svg
                    style={{ height: '50vh', aspectRatio: '16/9', maxWidth: '90%' }} // Se mantiene el style del componente original
