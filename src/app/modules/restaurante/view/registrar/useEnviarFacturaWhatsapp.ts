...
export const useEnviarFacturaWhatsapp = () => {
  ...
  const sendFactura = useCallback(async ({ telefono, urlPdf, nombreFactura, mensajePersonalizado }: SendWhatsappParams) => {
    if (!telefono) return;
    ...
    const mensaje = mensajePersonalizado || `Hola, le adjuntamos su factura de compra. ¡Gracias por su preferencia!`;
