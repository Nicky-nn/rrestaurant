...
export const DetalleAnomalia = ({ anomalia }: HistorialPedidoProps) => {
  if (!anomalia) {
    return (
      <Box mt={2}>
          ...
          <TableBody>
            <TableRow>
              ...
              <TableCell sx={{ whiteSpace: 'normal', wordBreak: 'break-word', color: 'error.main' }}>
                {(() => {
                  const texto = anomalia.descripcion || ''
                  const articulo = anomalia.nombre || ''
                  if (!texto || !articulo) return texto

                  // Limpiamos corchetes iniciales/finales
                  const cleanText = texto.replace(/^\[(FINANZAS:\s*)?/, '').replace(/\]\.?$/, '')

                  // Separamos por | o por .
                  const oraciones = cleanText
                    .split(/[|.]/)
                    .map((s) => s.trim())
                    .filter(Boolean)

                  // Buscamos todas las partes que incluyan el nombre del artículo
                  const coincidencias = oraciones.filter((s) =>
                    s.toLowerCase().includes(articulo.toLowerCase()),
                  )

                  return coincidencias.length > 0 ? coincidencias.join(' | ') + '.' : texto
                })()}
              </TableCell>
