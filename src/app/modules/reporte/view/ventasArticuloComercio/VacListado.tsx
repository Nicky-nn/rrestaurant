...
const VacListado: FunctionComponent<Props> = (props) => {
  ...
  const formatPreciosRegistrados = (
    precios: ReportePedidoVentasPorArticuloComercio['preciosRegistrados'],
  ) => {
    if (!precios || precios.length === 0) return '--'
    return precios.map((p) => `${formatAmount(p.precio)} x ${p.cantidad}`).join(' | ')
  }

  const formatPreciosRollo = (precios: ReportePedidoVentasPorArticuloComercio['preciosRegistrados']) => {
    if (!precios || precios.length === 0) return '--'
    return precios.map((p) => `${formatAmount(p.precio)}x${p.cantidad}`).join(' - ')
  }
  ...
  const generateLetterPDF = (data: ReportePedidoVentasPorArticuloComercio[]) => {
    ...
    const groupedByTipoArticulo = data.reduce(
      ...
      {} as Record<string, ReportePedidoVentasPorArticuloComercio[]>,
    )
  ...
  const generateRolloPDF = (data: ReportePedidoVentasPorArticuloComercio[]) => {
    ...
    return {
      ...
      pageMargins: [8, 8, 8, 8],
      content: [
        { text: 'Reporte Detallado', style: 'header' },
        {
        ...
        { text: '\n' },
        {
          table: {
            ...
            body: [
              ...
              ...Object.entries(groupedByTipoArticulo).flatMap(([tipoArticulo, items]) => [
                [
                  {
                    text: `Tipo Articulo: ${tipoArticulo}`,
                    colSpan: 4,
                ...
                ...items.flatMap((item) => [
                  [
                    ...
                    { text: `${item.nroVentas ?? 0}`, style: 'rollCellRight' },
