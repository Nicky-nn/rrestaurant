import { ContentCopyOutlined, ScreenSearchDesktopOutlined } from '@mui/icons-material'
...
const ArticuloSeleccion = forwardRef<SelectInstance<ArticuloProps>, Props>((props, ref) => {
  ...
  const genMontoValorColumna = useCallback(
    (item: ArticuloProps) => {
      ...
      const sigla = item.articuloPrecioBase?.monedaPrimaria?.moneda.sigla || ''
      const inv = item.inventario?.[0]
      const stock =
        item.verificarStock && inv ? `${numberWithCommasPlaces(inv.totalDisponible, nroDecimales)} ` : ''
    ...
    (resp: SingleValue<ArticuloProps>) => {
      if (resp) {
        if (!bloquearCodigosArticulo.includes(resp.codigoArticulo)) {
          ...
          toast.warning(`El articulo ${resp.codigoArticulo} ya se ha adicionado`)
        }
      } else {
        setArticulo([])
        onArticuloChange([])
    ...
    (resp: ArticuloProps[]) => {
      ...
      try {
        if (resp && resp.length > 0) {
          ...
          const bloqueados = resp.filter((item) => codigosBloqueadosSeguro.includes(item.codigoArticulo))

          if (bloqueados.length > 0) {
            const codigosErrores = bloqueados.map((b) => b.codigoArticulo).join(', ')
            // Usando tu toast en lugar de notDanger para mantener la consistencia
            toast.warning(`Los articulos ${codigosErrores} ya se han adicionado`)
          } else {
            setArticulo(resp)
            onArticuloChange(resp)
          }
  ...
  const getOptionValue = useCallback((item: ArticuloProps) => item.codigoArticulo, [])
  const noOptionsMessage = useCallback(() => 'Ingrese referencia -> Codigo articulo, nombre articulo', [])
      ...
      <Grid container spacing={0.5}>
        ...
        <Grid size={{ xs: 2, sm: 1, md: 1, lg: 1, xl: 1 }}>
          <ButtonGroup variant="text" aria-label="Opciones de busqueda">
            ...
            {permitirDuplicar && bloquearCodigosArticulo.length === 0 && articulo.length > 0 && (
              <IconButton
                ...
                disabled={disabled}
              >
                <Tooltip title={'Duplicar último artículo'} disableInteractive>
