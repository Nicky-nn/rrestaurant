...
export const seleccionArticuloInventarioReglas = (
  ...
  if (
    (loteProps.validarFechaVencimiento || reglas.validaLoteFechaVencimiento) &&
    !loteProps.disabled
  ) {
    if (
      item.lote &&
  ...
  if (reglas.valorMin) {
    if (item.precio < reglas.valorMin) {
      return [
        `Debe ingresar un valor (Precio / Costo) mayor o igual a ${numberWithCommasPlaces(reglas.valorMin)}`,
      ];
    }
  }
  ...
  if (reglas.validarCantidadStock) {
    ...
    if (articulo.verificarStock) {
      if (!inventario) {
        return [
          "Para verificar la cantidad por disponibilidad, deben existir datos de INVENTARIO ",
        ];
      }
      ...
      if (articulo.gestionArticulo === apiGestionArticulo.LOTE) {
        if (!item.lote || !inventario.lote) {
          return [
            "Error en stock disponible, debe seleccionar un LOTE que cuente con inventario > 0",
          ];
        }
