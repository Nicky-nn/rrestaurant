...
const EntradaRegistroRapido: FunctionComponent<Props> = () => {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: async (resp) => {
      toast.success(
        `Se ha registrado la entrada ${resp.codigo}, volviendo a la página principal...`,
      );
  ...
  useEffect(() => {
    if (isSuccess && monedas && monedas.length > 0) {
      const codigoMoneda = sinCaja
        ? moneda?.codigo
        : aperturaCajaActivo?.moneda?.codigo;
  ...
  return (
    <SimpleContainerBox maxWidth={"xl"}>
      ...
      <Breadcrumb
        routeSegments={[entradaRoutesMap.gestion, entradaRoutesMap.registro]}
      />
      ...
      <FormProvider {...form}>
        <Grid container spacing={1}>
          ...
          <Grid size={{ xs: 12, sm: 12, md: 7, lg: 8 }}>
            <SimpleCard
              title={"Productos / Articulos"}
              childIcon={<CategoryOutlined />}
            >
